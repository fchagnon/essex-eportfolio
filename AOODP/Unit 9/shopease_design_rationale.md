...
The diagram is color-coded to match the groupings this document arrives at:
...
entire purpose is to hold a User and a set of Products together, so that
...
**Resulting diagram element(s).** The diagram's color groups are the visual
...
| Requirement | Mechanism | Pattern Name | Diagram Color |
|---|---|---|---|
...
| Security | Hashed credentials; card data never touched by our code | N/A | (shown as absence) |
